...
> 五张地图的最后一张——也是最让人又爱又怕的一张。前四章你造了珠子、串起项链、贴上标签、照过镜子；这一章，你要学会**把项链拆下来重串**。
...
> **一颗提交造出来之后，SHA 永远是那个 SHA，内容永远是那个内容，parent 永远是那个 parent——它不可变。但一段"历史"是由\"分支标签指向哪颗珠子、每颗珠子的 parent 又指向谁\"共同决定的。你不能改一颗珠子，但你可以造一串近似的新珠子（内容相同或相似、parent 变了、SHA 就全变了），把标签指过去，让 `git log` 呈现出的\"历史\"完全变个样子。**
...
标签 `main` 挂在珠子 E 上。log 里显示的\"历史\"是 A → B → C → D → E。
...
看到了吗？**没有一颗珠子被修改**。发生的只是"造新珠子 + 挪标签"——第 2 章告诉过你，Git 里所有看似\"改历史\"的操作本质都是这两个动作。**这一章要做的，是把这个原理落地到具体的招式上：merge、rebase、squash、reset、revert、reflog——每一个都是"造新珠子 + 挪标签"的一种花式变奏。**

一句话总结这张图：**"历史重写"不是数据丢失，是标签重定向**。
...
好了，铺垫结束。下面依次讲 merge、rebase、squash 系列、reset、revert、reflog——每一招都建立在\"造新珠子 + 挪标签\"这条原理上。
...
### 5.3.1 情形 A：fast-forward——不打结，只挪标签

如果你要 merge 的分支和当前分支之间**没有分叉**（当前分支的位置是被合入分支的祖先），那么 Git **不会造任何新珠子**——它只是把当前分支的标签沿着项链前移到被合入分支的位置。
...
标签 main 从 B 顺着项链\"快进\"到了 E。没有 merge commit，没有分叉，log 里看起来就像 feature 分支从来没存在过一样——那些 commit 直接就是 main 历史的一部分。
...
- **旧的 X 和 Y 没有消失**——它们还在 `.git/objects/` 里，reflog 里也记得它们曾被 feature 标签指过。只是从 log 视角看，feature 上再也看不到它们了。
- **feature 的历史现在是一条直线**：A → B → C → D → X' → Y'，没有分叉、没有 merge commit，读起来像 feature 从 D 上直接长出来的一样。

**这就是 rebase 的全部\"魔法\"**——一次批量的"造新珠子 + 挪标签"。**没有一颗已存在的珠子被修改**（这条底层法则铁的不能再铁）。
...
你 rebase -i、squash/fixup 之后：
  ...
  X1-X8 都在你本地 objects 里，但 feature 标签指着新的 X1'-X3'。
```

你 `git push`——Git 检查发现：远端 feature 指着 X8，你本地 feature 指着 X3'，而**X8 不是 X3' 的祖先**（X3' 的祖先里根本没有 X8）。Git 拒绝 push：\"我没法从 X8 快进到 X3'，你告诉我怎么办？\"

**这不是 Git 犯傻**——它是在保护你：**\"这次推上去，远端 feature 上的 X1-X8 会从 log 里\"消失\"（其实还在 objects 里，但没有标签指着，早晚会被 gc 掉），已经拉过这条分支的同事会陷入混乱。你确定吗？\"**
...
- 用 `--force-with-lease` 之前先 `git fetch`——这样你的本地缓存反映的是刚刚的远端状态，最能挡住\"我离开的这几分钟里同事推了东西\"的意外。
...
2. `git fetch origin` 确认远端此刻确实是你 rebase 之前那 8 个 commit。
3. `git push --force-with-lease` 把整理后的 3 颗珠子推上去。
...
`git reset` 的核心动作只有一个：**把当前分支标签往回（或往任意位置）挪**。三个参数决定的是\"挪标签之外，工作目录和暂存区跟不跟着挪\"。
...
**`git reset --hard C`：挪标签 + 重置暂存区 + 重置工作目录。**
- main 从 E 挪到 C。
- 暂存区重置到 C。
- **工作目录也重置到 C**——D 和 E 的所有改动**从你的文件里被抹掉了**。
- 结果：一切像 D 和 E 从没存在过一样（除了 reflog 里的记录）。
- **用途**：\"我要完全丢弃最近几次 commit 和它们的改动——一键回到干净的 C 状态。\"
- **这是唯一一个会\"抹掉你工作目录里未提交改动\"的 reset**——是 reset 家族里唯一真正危险的一档，也是 Claude Code 官方推荐 deny 的对象（`Bash(git reset --hard*)` 建议设成 ask 或 deny）。
...
好了，本章你已经见过太多\"造新珠子、挪标签、旧珠子还在\"的场景。现在正式介绍那个让\"旧珠子还在\"这件事变得可用的机制——**reflog**。

### 5.7.1 reflog 是什么

**reflog 是本地操作的黑匣子**。每次 HEAD 或某个分支标签的位置发生变化，Git 都会在 `.git/logs/` 下的一个文本文件里记一行：\"什么时刻、什么操作、从哪个 SHA 挪到了哪个 SHA\"。
...
# main 标签挪回 a1b2c3d，3 颗 commit 回归
```
...
- `gc.reflogExpireUnreachable = 30` 天（reflog 里\"unreachable\"的条目——即没有任何标签能到达的珠子——保留 30 天）
...
```
git branch backup/<描述性名称>-$(date +%Y%m%d-%H%M)   ← 挂一张备份标签
git reflog                                              ← 看一下当前 HEAD 位置
```

第一步是**在当前珠子上挂一张\"我随时能回来\"的标签**——即使 reflog 过期了、gc 跑了，这张标签也还挂在原位置，那颗珠子就有引用不会被清扫。第二步是让你**心里对当前状态有底**，出问题时更容易找到目标 SHA。
...
**X 还在项链上，log 里能看到它**——但 X⁻¹ 把 X 的改动"平掉"了，最终项目状态相当于 X 没发生过。**这是"造新珠子 + 挪标签"原理最优雅的应用**：不改任何旧珠子、追加一颗新珠子来抵消错误。
...
|---|---|---|
| **物理动作** | main 标签从 D 挪回 B | 造一颗新珠子 X⁻¹ 追加到 D 之后 |
...
- 「我要撤销最近 3 个 commit，但改动都要保留下来（因为我要重新组织提交）。用 `git reset --soft HEAD~3`。执行前告诉我：这个操作会挪 main 标签、旧 3 个 commit 进 reflog；不会碰工作目录和暂存区里那 3 个 commit 的内容。」
...
| \"帮我从 reflog 里找回昨天的一个 commit\" | `git reflog` + `git reset --hard <sha>` | 读 `.git/logs/HEAD` 和相关文件（纯读）；找到目标 SHA 后 reset 一次——就是一次\"挪标签\"，被找回的珠子重新回到分支上 |

看懂这张表你会最终确认本章的核心：**没有任何一颗已存在的珠子被真正修改过——所有操作都归结为\"造新珠子 + 挪标签 + reflog 记账\"这三个动作的排列组合。** merge 是"造一颗 M + 挪一个标签"；rebase 是"批量造 N 颗新珠子 + 挪一个标签"；reset 是"只挪一个标签（也许附带清理暂存区/工作目录）"；revert 是"造一颗抵消珠子 + 挪一个标签"；amend 是"造一颗替代珠子 + 挪一个标签"。**五张变奏，一个主题。**
...
reset 系
git reset --soft <target>       # 只挪标签，暂存区/工作目录都不动
git reset --mixed <target>      # 挪标签 + 重置暂存区（默认，可省略 --mixed）
git reset --hard <target>       # 挪标签 + 重置暂存区 + 重置工作目录（危险）
...
reset 三态：
  ┌─ --soft  只挪标签           （改动全保留在暂存区）
  ├─ --mixed 挪标签 + 清暂存区   （改动留在工作目录）
  └─ --hard  挪标签 + 清全部     （危险：会抹掉未提交改动）
...
【地图 4 · 远程】（第 4 章）  ─── 镜子里的项链
    ...
    5 个命令碰网络：clone / fetch / pull / push / remote update
    non-fast-forward = Git 在按停按钮

【地图 5 · 历史】（第 5 章）  ─── 重串项链
    珠子不可变，项链可塑
    "重写历史" = 造新珠子 + 挪标签 + 旧珠子进 reflog
    merge 打结、rebase 重串、reset 挪标签、revert 造抵消珠子
    黄金规则：共享分支不重写
    reflog = 本地黑匣子，30 天保管期
...
**误解一：\"rebase 会丢代码。\"**
**恰好相反——rebase 是 Git 里最不容易\"丢\"代码的操作之一**。它做的全部事情是"逐颗重放你的改动、造出内容相同但 parent 换了的新珠子\"——你的每一处改动都被完整地搬运到了新的珠子里。真正会\"丢\"东西的操作是 `reset --hard` 时抹掉工作目录里未提交的改动。这两件事经常被混淆——因为 rebase 中途遇到冲突时 log 里那些旧 SHA 突然\"看不到了\"，让人以为它们消失了；实际上它们还在 objects/，reflog 里也全都记着，随时可以 `git reset --hard <旧 SHA>` 一键回来。**\"rebase 会丢代码\"是新手最容易得的病，也是让很多人一辈子不敢用 rebase 的心理阴影**——治愈它的药方就是这一章讲的两件事：（1）rebase 前挂 backup 分支；（2）出问题 `git reflog` 一看，救回来往往只需要一条命令。**你越用越会发现：rebase 不是烫手的山芋，它就是"造新珠子 + 挪标签"的一种批量花式变奏。**
...
**revert 不是"回滚"，是"造一颗抵消珠子"**——两者在\"最终效果\"上可能相同，但\"如何达到\"的物理路径完全不同，也决定了对共享分支的安全性完全不同。回滚（reset --hard）是\"标签往回挪，旧珠子成为野珠子\"——在共享分支上做就是灾难。revert 是\"往前造珠子\"——log 里错误珠子 X 依然存在，旁边多了 X⁻¹ 撤销它。**结果就是 revert 的历史更长（多了一颗珠子），但每一步都是可追溯的，都是"往前"造，没有任何珠子被"回收"**。这是**共享分支上唯一安全的撤销方式**。而且 revert 可以被再次 revert（"撤销这次撤销"），保持了完整的可追溯性——这对生产环境的事故回溯至关重要。**记住这条口诀：个人分支撤销用 reset（或 rebase），共享分支撤销用 revert。**
...
- 用\"造珠子 + 挪标签\"这条原理推导出几乎所有 Git 命令的物理效果。
