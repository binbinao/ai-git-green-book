...
1. **它会把"做了什么"当成"为什么做"**——因为它只能看 diff，看不到你脑子里的意图。message 里说"把 decimal 改成 Decimal"，但读者想知道的是"为什么改"。**body 的"为什么"AI 能起草，但只有你知道它猜得对不对**——它会从 diff 和上下文里猜一个理由，猜对了你采纳，猜偏了只有你能纠正（6.8 的指令模板里那句"diff 里看不出为什么就问我"说的就是这个分工）。
...
红黄绿三色：
  ...
  🔴 不可逆销毁 / 覆盖共享远端 / 涉及凭证 —— 流程级确认，你亲手来
  push 之前人眼扫 diff（debug 遗留、假数据、敏感文件）—— 不分颜色的铁律
```
